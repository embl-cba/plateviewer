...
public abstract class MultiWellHdf5CachedCellImage< T extends RealType< T > & NativeType< T > > extends MultiWellCachedCellImage< T >
{
	...
	public MultiWellHdf5CachedCellImage( List< File > files, String namingScheme, int numIoThreads )
	{
		this( files, namingScheme, numIoThreads, null, null );
	}

	public MultiWellHdf5CachedCellImage( List< File > files, String namingScheme, int numIoThreads, String channelName )
	{
		this( files, namingScheme, numIoThreads, channelName, null );
	}

	public MultiWellHdf5CachedCellImage( List< File > files, String namingScheme, int numIoThreads, String channelName, int resolutionLevel )
	{
		this.files = files;
		...
		if ( resolutionLevel == 0 )
			this.hdf5DataSetName = channelName + "/image";
		else
			this.hdf5DataSetName = channelName + "/image_scale" + ( resolutionLevel + 1 );

		setCachedCellImgDimensions( singleSiteChannelFiles );

		setImagePropertiesUsingJHdf5( files.get( 0 ) );

		setSingleSiteChannelFiles( files, hdf5DataSetName, namingScheme );

		wellNames = multiWellChannelFilesProvider.getWellNames();

		setMultiSiteLoader( numIoThreads, singleSiteChannelFiles );

		setCachedCellImg();
	}

	public MultiWellHdf5CachedCellImage(
	...
	public void setSingleSiteChannelFiles( List< File > files, String hdf5DataSetName, String namingScheme )
	{
		multiWellChannelFilesProvider = getMultiSiteChannelSourceGenerator( files, hdf5DataSetName, namingScheme );
	...
	private void setMultiSiteLoader( int numIoThreads, ArrayList< SingleSiteChannelFile > singleSiteChannelFiles )
	{
		loader = new MultiSiteLoader( singleSiteChannelFiles, numIoThreads );
	}
